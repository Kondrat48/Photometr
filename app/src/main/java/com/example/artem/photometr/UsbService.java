...
//            }
    ...
    private boolean serialRW(int adr) throws InterruptedException {
        ...
        do {
            ...
            } while (error.equals("") || cmCount > 5);

            if (!error.equals("")) continue;
            serialPort.write(new byte[]{(byte) (char) 0xAA});
            wait(1000);
            System.arraycopy(readResult, 0, frez, 0, 64);
            result = true;
    ...
    public boolean readContent() {
        boolean result = false;
        int i, j, rpt = 0, blockCount = slotCount * 3 / 64, adr;
        ...
        do {
            ...
            try {
                for (j = 0; j < blockCount; j++) {
                    ...
                    for (i = 0; i < 64; i++) btArr[i + j * 64] = frez[i];
                }
                for (i = 0; i < slot.length; i++) {
                    slot[i].number = btArr[i * 3 + 1] << 8 + btArr[i * 3];
                    slot[i].enabled = btArr[i * 3 + 2] == 1;
                }
                result = true;
